...
## v0.23.366 — Always ผูกกับ folder ไม่ใช่ id ที่เปลี่ยนทุกครั้งที่เพิ่ม project ใหม่

ต่อจาก 365 เจ้าของถามว่า Always ที่กดไปครอบแบบไหน — ครอบทั้ง project (เขียน sub-folder
ไหนก็ไม่ถามอีก) หรือครอบเป็นราย folder คำตอบคือ**ต้องการแบบครอบทั้ง project** และตอนตรวจ
พบว่ามีปัญหาซ้อนอยู่อีกชั้น

grant เคยเป็น `(projectID, toolID, actionClass)` และ `project.id` เป็น UUID ที่
`Project.init` สร้างใหม่ทุกครั้งที่เพิ่ม folder เข้า registry **ลบ project แล้วเพิ่มกลับ
= Always ที่เคยให้ไว้กลายเป็นเศษที่ไม่มีใครอ่านเจอ โดยไม่มีอะไรบอก** ไฟล์ของเจ้าของวันที่
2026-08-22 มี 6 แถว มีแถวเดียวที่ชี้ไป project ที่ยังอยู่จริง อีกตัวละครหนึ่งมี project list
ว่างเปล่าแต่ยังมี grant ค้างอยู่สองแถว และร่องรอยบอกว่า Always ของ Second-Brain ถูกให้ไว้
วันที่ 08-20 หายตอน registry ถูกเขียนใหม่วันที่ 08-21 แล้วต้องให้ใหม่วันที่ 08-22

### ที่แก้

- **หนึ่ง key ต่อหนึ่ง project และ key คือ path ของ folder** `StandingGrant(projectPath:)`
  แถวเดียวจบ ไม่มี tool ไม่มี class — แปลว่า "อนุมัติให้ทำงานใน project นี้"
  `CanonicalPath` เป็นที่เดียวที่ path กลายเป็น key (`standardizedFileURL`, ตัด `/` ท้าย)
  เขียน path คนละแบบจึงกลายเป็นสอง grant ไม่ได้
- **ลบ project แล้วเพิ่มกลับ Always ยังอยู่** เพราะ key ไม่ได้ผูกกับ id ที่เปลี่ยน
- **session grant ยังเก็บ tool และ class ไว้เหมือนเดิม** "Once" ต้องแปลว่าเฉพาะสิ่งที่ถาม
  เท่านั้น ความไม่สมมาตรตรงนี้ตั้งใจ — เฉพาะครึ่งที่จำถาวรเท่านั้นที่เป็นหนึ่งแถวต่อ project
- **สิ่งที่ Always ยังไม่ครอบ ไม่เปลี่ยน** `noGrantMaySkipThis` ยังถูกถามก่อน grant เสมอ
  ลบ/เขียนทับ, แก้ git history, ลง dependency, ส่งออกนอกเครื่อง, ทำใน browser และเปิด
  folder นอก project ยังถามทุกครั้ง และ tool ที่ไม่อยู่ใน `allowedTools` ของ project
  ก็ยังถาม
- **แก้ข้อความบนการ์ดที่สัญญาเกินจริง** เดิมเขียนว่า "Always means you won't be asked for
  this project again" ซึ่งภายใต้ขอบเขตใหม่ไม่จริง ตอนนี้บอกตรง ๆ ว่าครอบการอ่านและเขียน
  ในโปรเจกต์นี้ ส่วนการลบ การติดตั้ง และอะไรที่ออกนอกเครื่องยังถามอยู่

### Migration

`load` ลองอ่านรูปแบบใหม่ก่อน ถ้าไม่ได้จึงอ่านรูปแบบเก่าแล้วแปลง `projectID` ผ่าน
`projects-<id>.json` ของตัวละครนั้นเป็น path แถวที่ project หายไปแล้วจะ**ถูกทิ้ง** เพราะ
ไม่มีทางรู้ path ของมัน และการเดา path ให้เท่ากับให้สิทธิ์ folder ที่ไม่มีใครระบุ ผลลัพธ์
ถูกเขียนกลับลงไฟล์ครั้งเดียว การตัดแถวทิ้งเกิดเฉพาะตอน migrate เท่านั้น ไม่เกิดตอนโหลด
ปกติ เพราะ external drive ที่ยังไม่ได้เสียบไม่ควรทำให้สิทธิ์หายเงียบ ๆ

ขับจริงบนไฟล์ของเจ้าของ 2026-08-22: 6 แถวเหลือ 1 แถวเป็น
`/Users/Olarn/AllWorks/Second-Brain` และ id ที่ตายแล้วทั้งสี่หายไป อีกสองตัวละครไม่เหลือ
อะไร ซึ่งถูกต้อง — ตัวหนึ่ง project list ว่าง อีกตัว grant ชี้ไป id ที่ registry ไม่มีแล้ว
จากนั้นสั่งเขียนไฟล์ใน vault ผ่านการ์ดไม่ขึ้นเลย 1475 tests, 0 failures
