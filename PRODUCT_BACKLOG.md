...
- [x] **เปลี่ยนคำว่า "your Claude Code default" เป็นคำกลาง ๆ** (เจ้าของทักระหว่างทำ
  2026-08-21) — เมนู Model/Effort, ประโยคที่เธอพูดตอนกลับไปใช้ default, และข้อความ
  ของ `/model` `/effort` ทั้งหมดเคยเอ่ยชื่อ Claude Code ซึ่งตอนนี้ผิดเมื่อเลือก
...
## OpenCode ช้าเพราะอะไร และแก้ตรงไหน (v0.23.353)

เจ้าของถามว่าตอน resume session แอปใช้ cold หรือ warm และช้าแบบนี้แก้ได้ไหม
คำตอบคือ **cold ทุกเทิร์น** (`OpenCodeProvider` สร้าง `Process()` ใหม่ทุกครั้ง ต่อ
session ด้วย `--session` เฉย ๆ ไม่มี warm process แบบฝั่ง Claude) — แต่**นั่นไม่ใช่
สาเหตุที่ช้า** วัดจริงบนเครื่องนี้ (opencode 1.18.15 + qwen3.8:27b-mlx, 2026-08-21):

| | |
|---|---|
| opencode binary startup | **0.3 วิ** |
| โหลดโมเดล 26.9GB | ~27 วิ |
| prefill prompt 13,127 tokens (cache เย็น) | **~116 วิ** (~126 tok/s) |
| prefill เดิม (cache อุ่น) | **0.3 วิ** |
| generate ~25 tokens | 2–3 วิ |

**เพราะฉะนั้นการทำ warm process จะประหยัดได้ 0.3 วิ จาก 116 วิ — ไม่คุ้มสร้าง**
ตัวที่กินเวลาคือ opencode ยัด system prompt + tool definitions 13k tokens ทุกเทิร์น
แล้วโมเดลต้อง prefill ใหม่ และ **ollama unload โมเดลทิ้งหลังไม่ใช้ 5 นาที** (ค่า
default) ซึ่งทำให้ cache หายไปด้วย — คนที่เปิดแอปคุยเป็นพัก ๆ จึงเจอ 116 วิ *ทุกครั้ง*
ไม่ใช่แค่ครั้งแรก

**ตัววัดที่ชี้ขาด** (เทิร์นจริงในแอป จับเวลาจริง):
- โมเดลถูก unload ไปแล้ว → **180 วิ**
- ยิงต่อทันทีจากเทิร์นก่อนหน้า → **34 วิ**
- **pin โมเดลไว้ (`keep_alive: -1`) แล้วเว้นว่าง 7 นาที → 34 วิ** ← ตัวนี้คือตัวแก้จริง

- [x] **เอกสาร/ตัวเลขข้างบน** เก็บไว้ใน `VendorWarmUp.swift` ด้วย เพราะ "ทำ warm
  process สิ" เป็นข้อเสนอที่ฟังดูถูกเสมอ และจะถูกเสนอใหม่เรื่อย ๆ ถ้าไม่มีตัวเลขค้านไว้
- [x] `VendorRuntime.warmUp` + `shouldWarmUp` — แอปยิงเทิร์นเปล่า ๆ ("ready") ใน
  background ตอนเลือก OpenCode เพื่อจ่ายค่า prefill ตอนไม่มีใครรอ
  - ใช้ flag เดียวกับเทิร์นจริงเป๊ะ (`--dir`, `--model`) เพราะ prefix ที่ cache ไว้
    สร้างจากสองอย่างนี้ — warm ผิดโฟลเดอร์ = อุ่น prompt ที่ไม่มีเทิร์นไหนส่ง
  - **ไม่ใส่ `--session`** เพื่อไม่ให้ "ready" โผล่ในห้องแชทที่คนกำลังคุยอยู่
  - ยิงครั้งเดียวต่อ (ค่าย, model, โฟลเดอร์) และ**ไม่ยิงตอนมีเทิร์นค้างอยู่** ไม่งั้น
    warm-up กับคำถามของคนใช้จะไปต่อคิวกันบนโมเดลตัวเดียว ช้าทั้งคู่
  - **ยังพิสูจน์ไม่ได้ว่าช่วย** — ตอน drive จริง warm-up จบไปแล้ว 4 นาทีก่อนคำถามจริง
    cache เย็นไปแล้ว เทิร์นนั้นเลยยัง 180 วิ มันจะช่วยเฉพาะเมื่อคำถามมาไม่นานหลัง warm
    เขียนไว้ตรง ๆ ดีกว่าปล่อยให้รอบหน้าเข้าใจว่ามันแก้ปัญหาไปแล้ว
- [x] `Secretary.workingDirectory` เปิด public — warm-up ต้องรู้ว่าเทิร์นหน้าจะรันที่ไหน

**ที่ไม่ได้ทำ พร้อมเหตุผล**: ลด prompt ด้วย custom agent ที่ปิด tool ทำได้จริง
13,127 → **8,693 tokens (−34%)** แต่ต้องปิด write/edit/bash/grep ทิ้ง = แลกความสามารถ
เกือบทั้งหมดเพื่อประหยัด ~40 วินาทีครั้งเดียว ไม่คุ้ม (`--pure` ไม่ช่วยเลย ยัง 13,127
เท่าเดิม แปลว่ามาจาก opencode เอง ไม่ใช่ plugin)
...
**สองข้อที่แก้ตามหลังในรอบเดียวกัน**

- **เลขเวอร์ชันชุดใหม่ต้องมีเทสเฝ้า** charter เขียนไว้ว่า "ถ้าเอกสารใหม่อ้างเลขเวอร์ชัน
  ให้เพิ่มแถวใน `versionMentions`" — รอบแรกเขียนเลขลงสองที่ใหม่โดยไม่ได้เพิ่มแถว
  ซึ่งคือ stale เงียบ ๆ รอบหน้าที่ bump พอดี ตอนนี้ `code/README.md` ไม่เขียนเลขแล้ว
  (ซ้ำกับ Requirements และกับ README หลัก) ส่วน `docs/FEATURES.md` เพิ่มแถวพร้อม
  pattern ที่ **ยึดกับข้อความ** (`State of the product at v…`) เพราะเทสบังคับว่า
  *ทุก* match ต้องตรงกับ `AppVersion.current` และในไฟล์มี `v0.14.262` อยู่ในตาราง
  Superseded — pattern แบบ `v(\d+\.\d+\.\d+)` เปล่า ๆ จะแดงทันที
  - พิสูจน์ว่าเทสเฝ้าจริง: แก้เลขในไฟล์เป็น 0.23.999 แล้วรัน ได้แดงตรงบรรทัดนั้น
    แล้วคืนค่ากลับ (เทสที่เขียวทั้งก่อนและหลังไม่ได้คุมอะไร)
- **เขียน `code/README.md` ใหม่ทั้งที่อ่านไปแค่ ~60%** อ่านบรรทัด 1–120 กับ 300–439
  แล้วเขียนใหม่ ตรงกลาง ~180 บรรทัดจึงหลุดไปโดยไม่ได้อ่าน ซึ่งเป็นความผิดพลาดแบบที่
  หัวข้อ Comments ของ charter เขียนไว้ตรง ๆ ("bulk-stripping a file you have not read
  destroys knowledge") กลับไปอ่านแล้วเอาที่ยัง**จริง**กลับมา:
  - Git adapter ปลอดภัยโดยโครงสร้าง — `/usr/bin/git` ด้วย absolute path, argv เป็น array
    (ไม่มี shell), argument vector ตายตัวต่อ operation, environment ขั้นต่ำ,
    timeout 15 วิ, output cap 128KB, เช็ค `.git` ก่อนรัน (ตรวจกับโค้ดแล้วทุกข้อ)
  - `--allowedTools` คือ pre-approve **ไม่ใช่การจำกัด** พร้อมชุด read-only ตามจริง
    และเหตุผลที่ต้องใช้ `--disallowedTools` แทน
  - MCP ไม่ถูกกั้นด้วย allowlist, และ child ได้ `PATH` ของ login shell
  - caps ของบทสนทนา ตรวจกับ `Secretary.swift` แล้วเป็น 4,000 / 16,000 / 60,000 /
    200,000 **ไบต์** (README เดิมเขียนเป็น KB ซึ่งปัดผิดเล็กน้อย)
  - ที่ **ไม่**เอากลับ เพราะไม่จริงแล้ว: แผง Debug, ทางเลือก API key, ไอคอนที่สร้างจาก
    `character.png`, และประโยคที่ว่า grant การเขียนไฟล์อยู่แค่ session (0.15.271 แก้ไปแล้ว)
