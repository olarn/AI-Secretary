...
ทำ #0 ก่อนเสมอ เพราะมันเป็นตัวกำหนดว่าอีกสิบข้อเป็นงานชุดเดียวหรือสองชุด
...
- [ ] **ด่านแรกก่อนทำข้ออื่น: glass บนหน้าต่างที่ไม่เป็น key** — AppKit ถอด accent ออกจาก
  tinted control เมื่อหน้าต่างไม่ใช่ key มาแล้ว (คือที่มาของ `PanelToggleStyle`) ถ้า `.glass`
  ประพฤติแบบเดียวกัน ปุ่มจะดูตายตลอดเวลา **ประเมินจากโค้ดไม่ได้ ต้องเปิดแอปแล้วดู**
  จึงทำบับเบิล + ปุ่ม footer ก่อนแล้วขับจริง ถ้าไม่ผ่านจะเสียแค่สองจุด ไม่ใช่สิบจุด
