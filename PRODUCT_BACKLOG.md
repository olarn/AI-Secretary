...
- [x] ค่าเริ่มต้น: Projects ซ้ายสุด แล้ว Profile แล้ว Settings อยู่ขวา
- [x] ตอน mirror (ซ้ายขวา / บนล่าง) ลำดับต้องสลับให้ถูก — แถวย้ายไปชิดอีกขอบ ลำดับจึงกลับด้าน เพื่อให้แต่ละปุ่มอยู่ห่างจาก "ขอบนอก" เท่าเดิม ถ้าคงลำดับตรงตัวไว้ ปุ่มทุกตัวจะไปอยู่ใต้นิ้วคนละที่แล้วแต่ว่ากล่องพลิกทางไหน
  - เป็นกฎเดียวกับแถวปุ่มปิด/ขยายด้านบนที่กลับลำดับเพื่อให้ปุ่มปิดอยู่ริมนอกเสมอ
  - กฎอยู่ใน `footerOrder` ที่ `SecretaryCore` มีเทสคุมทั้งสองด้าน ไม่ใช่ array ตายตัวในไฟล์ view — "สลับแล้วถูกไหม" ไม่ควรต้องตอบด้วยการเพ่งภาพหน้าจอ
