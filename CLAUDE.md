...
- **ตัวเลือกในแชทต้องมาจากรูปแบบที่เรากำหนด ห้ามเดาจากร้อยแก้ว** — โมเดลเขียน list ตลอดเวลา
  ...
  - ปุ่มลูกศรในกล่องพิมพ์มีสองความหมาย (เลือกตัวเลือก / เรียกประวัติ) — ต้องกำหนดลำดับให้ชัด
    ตอนมี picker อยู่ ลูกศรเป็นของ picker เท่านั้น
- **`.onKeyPress` ไม่ได้เห็นปุ่มลูกศร** — `TextField` กินไปใช้เลื่อน caret ก่อน (Return ผ่าน แต่ลูกศรไม่ผ่าน)
  ต้องใช้ `NSEvent.addLocalMonitorForEvents` ซึ่งเห็นก่อน responder chain
