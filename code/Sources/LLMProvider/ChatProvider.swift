...
public protocol ChatProvider: Sendable {
    /// `model` and `effort` are `Option`: absent means the user hasn't chosen,
    /// so the backend's own configured default should stand. Overriding it
    /// would quietly give someone a different model than the one they set up.
    func stream(
