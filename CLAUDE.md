...
- resize chat window ได้เองด้วยการลาก grip ที่มุมล่างด้านตรงข้ามหาง (กว้างและสูง), minimum เท่าขนาด default, max ไม่เกินหน้าจอ
- มี icon "←|→" (กว้างขึ้น) และ "→|←" (แคบลง) เรียงข้างปุ่มปิด chat window โดย icon ทั้งสองเล็กกว่าปุ่มปิด 30%
   - กด 1 ครั้ง = 1 step: x1 → x2 → x3 แล้ว disable (และถอยกลับทีละ step ในทางกลับกัน)
