...
- [x] **⌘H และ Esc ต้องทำงานแม้แอปไม่ได้เป็นหน้าต่างหน้าสุด** (เจ้าของสั่งเอง หลังรับทราบผลข้างเคียงแล้ว)
  ...
  - ใช้ `RegisterEventHotKey` ของ Carbon — ทางเดียวที่ไม่ต้องขอสิทธิ์ Accessibility (ต่างจาก `CGEventTap`) และเป็นทางเดียวที่ **กลืน** คีย์ ส่วน `addGlobalMonitorForEvents` ได้ยินแต่กลืนไม่ได้ ⌘H จะไปซ่อนแอปที่อยู่หน้าสุดด้วย
  - **Esc จดทะเบียนเฉพาะตอนกล่องแชทเปิด** — Esc เป็นคีย์ที่คนใช้ตลอด (ปิด dialog, ออก full screen, จบสไลด์โชว์) ยึดค้างไว้ทั้งที่กล่องไม่ได้เปิดคือพังของทั้งเครื่องแลกกับหน้าต่างที่ไม่ได้โผล่ กติกาว่ายึดคีย์ไหนเมื่อไหร่อยู่ใน `GlobalShortcut` ที่ `SecretaryCore` มีเทสคุม ไม่ใช่ register สองบรรทัดฝังในโค้ดหน้าต่าง
  - ⌘H ยึดตลอดและเป็น toggle — ถ้าไม่ toggle พอซ่อนจากแอปอื่นแล้วจะไม่มีทางเรียกกลับนอกจากเมนูแถบสถานะ และต้องสั่ง `setCharacterVisible` เองเพราะ ⌘H จากนอกแอปไม่ได้ผ่านเมนู
  - **อย่าใช้การ register ซ้ำเป็นเครื่องมือวัดว่าใครถือคีย์อยู่** — เขียนโพรบไปแล้วรอบหนึ่ง macOS ยอมให้อีก process จดคีย์เดียวกันได้ มันตอบ FREE ทั้งตอนที่แอปถืออยู่และไม่ถือ วัดได้ทางเดียวคือดูพฤติกรรมจริง: เปิดแถบค้นหาของ Safari (⌘F) แล้วกด Esc — กล่องเปิดอยู่ แถบค้นหาต้องยังอยู่ (เรากลืน) กล่องปิดแล้ว แถบค้นหาต้องหาย (เราคืนคีย์)
